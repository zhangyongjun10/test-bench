...
### Case 删除行为修正
- Case 删除改为物理删除：删除时会先清理 `scenario_agents` 关联，再删除 `scenarios` 主记录，不再只做软删导致底层残留数据。
- 若该 Case 仍被执行、比对或回放记录引用，后端会显式返回删除失败，避免前端误以为已经删除成功。
- 删除失败提示统一改为中文业务文案，避免接口直接向前端暴露英文技术描述。

### Trace 展示顺序修正
- Trace 接口补充返回 `start_time_ms` 与 `end_time_ms`，供前端在执行详情页与回放详情页做一致的稳定排序展示。
- Trace 排序规则进一步收口到后端：后端改为按 `start_time` 到秒分组、同秒再按 `end_time` 和原始顺序稳定排序，前端不再自行重排，避免前后端双重排序导致结果不一致。


## 2026-04-23

### Trace 回放指标与展示
- Trace 回放顶部新增总耗时、总吞吐量和输出吞吐量摘要指标；总吞吐量按 `(输入 Tokens + 输出 Tokens) / 总耗时` 计算，输出吞吐量按 `(output_tokens - 1) / 首 token 后时长` 计算。
- 每个 OpenAI LLM span 新增总吞吐量和输出吞吐量展示，便于同时观察端到端处理速度和首 token 之后的纯输出速度。
- Trace 详情中的 LLM 输入和输出新增一键复制按钮，支持直接复制原始请求体和响应体内容。
- 执行详情页与回放详情页的 Trace 摘要和 span 指标文案统一修正为正常中文，修复吞吐量标签显示乱码的问题。

### Trace 指标计算
- Trace 摘要计算新增总耗时汇总，并继续保留平均 TTFT、加权平均 TPOT 和总 Tokens 聚合。
- LiteLLM PG timing 补值链路新增总吞吐量与输出吞吐量计算，其中 TPOT 和输出吞吐量优先复用 LiteLLM PG 同源的 `endTime - completionStartTime` 口径。
- Trace API 响应和前端类型补充总耗时、总吞吐量与输出吞吐量字段，供执行详情页和回放详情页统一消费。
...
- Trace 回放中单个 span 的 `耗时`、`TTFT`、`TPOT` 展示格式已统一；有小数时最多保留 2 位，整毫秒不再强行补 `.00`。
