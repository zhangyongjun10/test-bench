...
- ✅ **基线来源**：用户在场景上手动设置基线，`baseline_tool_calls` 存过程，`baseline_result` 存结果
...
- 新增 `ComparisonService.detailed_compare()` 方法实现完整比对：
  ...
  2. **过程比对**（如果 `baseline_tool_calls` 不为空）：
     ...
     - 如果差异在容忍范围内 → **最优匹配不要求顺序**：
       ...
       - 未匹配到的 tool 得 0 分
     - **对每个匹配对，使用两阶段策略**：
       - **JSON 预处理**：如果输入输出看起来是 JSON（以 `{` 或 `[` 开头），尝试解析后用标准格式重新序列化，消除格式差异
       - 算法粗筛：编辑距离计算相似度 = `(input_sim + output_sim) / 2`
       - `> 0.9` → 满分 1.0，跳过 LLM
       - `0.5 ~ 0.9` → 调用 LLM 验证语义一致性
       - `<= 0.5` → 0 分
...
| 操作 | 文件路径 | 说明 |
|------|---------|------|
| 修改 | `backend/app/domain/entities/scenario.py` | 新增 `baseline_tool_calls`, `process_threshold`, `result_threshold`, `tool_count_tolerance` 字段 |
...
| 修改 | `frontend/src/pages/ExecutionDetail.tsx` | 新增比对详情展示 |
